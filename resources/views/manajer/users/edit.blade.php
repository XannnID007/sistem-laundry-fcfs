...
@section('content')
    <div class="page-inner">
        <div class="page-header">
            <h4 class="page-title text-primary fw-bold">Edit Data User</h4>
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">Form Update User</div>
                    </div>
                    <form action="{{ route('manajer.users.update', $user->user_id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                            ...
                            <div class="form-group @error('password') has-error @enderror">
                                <label for="password">Password Baru <small class="text-muted text-warning">(Kosongkan jika
                            ...
                            <div class="form-group @error('role') has-error @enderror">
                                ...
                                <select class="form-control" id="role" name="role" required
                                    ...
                                    <option value="Manajer" {{ old('role', $user->role) == 'Manajer' ? 'selected' : '' }}>
                                        Manajer</option>
                                    <option value="Hotel" {{ old('role', $user->role) == 'Hotel' ? 'selected' : '' }}>
                                        Hotel</option>
                                    <option value="Laundry" {{ old('role', $user->role) == 'Laundry' ? 'selected' : '' }}>
                                        Laundry</option>
                                </select>
                                @error('role')
                                    <small class="form-text text-danger">{{ $message }}</small>
                                ...
                                <select class="form-control" id="hotel_id" name="hotel_id">
                                    ...
                                    @foreach ($hotels as $hotel)
                                        <option value="{{ $hotel->hotel_id }}"
                                            {{ old('hotel_id', $user->hotel_id) == $hotel->hotel_id ? 'selected' : '' }}>
                                            {{ $hotel->nama_hotel }}
                                        </option>
                                    @endforeach
                                </select>
